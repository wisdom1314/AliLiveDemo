...
//    [self addBackgroundNotifications];
    ...
    int start = [self startPush];
    if (ret != 0) {
        [self showPusherStartPreviewErrorAlert:start isStart:YES];
        return;
    }
}
...
- (int)setupPusher {
    ...
    if(self.isUserMainStream) {
        ...
        self.pushConfig.audioChannel = 1;
        self.pushConfig.audioSampleRate = AlivcLivePushAudioSampleRate44100;
...
- (CGRect)getFullScreenFrame {
    ...
    if (IPHONEX) {
        // iPhone X UI适配
        frame = CGRectMake(0, 88, AlivcScreenWidth, AlivcScreenHeight-88-57);
    }
