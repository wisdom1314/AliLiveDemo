...
@interface AlivcLivePushReplayKitConfigViewController () <UITableViewDelegate, UITableViewDataSource>
...
- (void)setupSubviews {
    ...
    self.paramTableView.frame = CGRectMake(retractX,
                                           ...
                                           viewWidth,
                                           CGRectGetMinY(self.publisherButton.frame) - CGRectGetMaxY(noticeLabel.frame) - 300);
...
- (void)publiherButtonAction:(UIButton *)sender {
    ...
    if (audioStatus == AVAuthorizationStatusNotDetermined) {
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"当前没有麦克风权限，请在设置中打开麦克风权限" delegate:self cancelButtonTitle:@"去设置" otherButtonTitles:@"取消", nil];
        [alert show];
        return;
    }
